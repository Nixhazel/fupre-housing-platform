...
interface RoommateDetailClientProps {
	listingId: string;
}

export function RoommateDetailClient({ listingId }: RoommateDetailClientProps) {
	...
	const handleCopy = async (text: string, field: 'email' | 'phone') => {
		try {
			...
			toast.success(
				`${field === 'email' ? 'Email' : 'Phone'} copied to clipboard`
			);
									...
									{showContact && owner && isAuthenticated ? (
										...
										<motion.div
											...
											{isOwnListing ? (
												...
												<>
													<div className='p-4 rounded-lg bg-linear-to-r from-primary/10 to-primary/5 mb-4'>
